...
contract Encoding {
    // the abi.encode() is way of telling sol to encode the str and convert em into bin form
    //abi.encodePacked() is same as above but w/o the empty initial 0's in the mem addr (w/o Padding)
    //we can kinda think this encodePacked as a compresser --> gives us much smaller byte obj and its good for saving gas
    ...
    //This is as similar as the above fn
    ...
    //lly we can decode the encoded bytes(bin) back to str
    ...
    // This doesn't work!
    //coz we passed the multi encoded packed one and  sol doesn't know how to decode
    ...
    // for you. back to when we withdrew ETH from our raffle:

    function withdraw(address recentWinner) public {
        (bool success, ) = recentWinner.call{value: address(this).balance}(""); // keep the data(calldata) bit empty, just updating only the vals
